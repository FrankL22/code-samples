...
int RunSingleSimulation(int n, int m, int x, std::string& log, int simNum, bool needLog)
{
    ...
    if (needLog) log += "prize(s) were at door ";
    ...
    if (needLog)
    {
        log += "first pick was door " + std::to_string(firstPick)
            + (acc[firstPick] == 1 ? ", which had a prize;\n" : ";\n");
    }

    // game host randomly removes x doors
    if (needLog) log += "game host removed door ";

...
int RunSingleSimulation(int n, int m, int x, std::string& log, int simNum)
{
    log += "@@ Sim #" + std::to_string(simNum) + " @@\n";
    ...
    log += "first pick was door " + std::to_string(firstPick) + (acc[firstPick] == 1 ? ", which had a prize;\n" : ";\n");

    // game host randomly removes x doors
    log += "game host removed door ";
    ...
    for (int i = 0; i < x; i++)
    {
        ...
        while (rep[index] == firstPick ||   // Cannot remove the picked door
            (prizeRemaining < 2 && acc[rep[index]] == 1)) // Must leave at least one prize left
        {
    ...
    log += "second pick was door " + std::to_string(secondPick) 
        + (acc[secondPick] > 0 ? ", which was a HIT!\n\n" : ", no hit.\n\n");

    // check if second pick is a hit
    return (acc[secondPick] > 0) ? 1 : 0;
}
...
float RunSimulations(int n, int m, int x, int k)
{
    // Run params legitimacy check, as per game rules
    if (n < 3) return -1.0f; // Cannot have less than 3 doors
    if (m > n) return -1.0f; // Cannot have more prizes than doors
    if (x > n - 2) return -1.0f; // Game host must leave at least 2 doors to swap between
    if (m < 1 || x < 0 || k < 1) return -1.0f; // Numbers validity check

    // Log simulation basic info
    std::string logText = "Running " + std::to_string(k) + " simulations for " + std::to_string(n) 
    ...
    for (int i = 0; i < k; i++)
    {
        hitCount += RunSingleSimulation(n, m, x, logText, i + 1);
...
int main()
{
    ...
    while (true)
    {
        std::cin >> n >> m >> x >> k;
        std::cout << "\n";
        float prob = RunSimulations(n, m, x, k);
        float oldProb = (float)m / (float)n;
        if (prob < 0.0f)
        {
            std::cout << "\nSimulation couldn't be properly completed due to an error. Check output log for more info.\n";
        }
        else
        {
